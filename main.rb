...
class Prose < Sinatra::Base
    ...
    ["/flow", "/f"].each do |path|
        post "#{path}", auth: :writer do
            ...
            $writer_m.update_stats(writer, params[:highest_wpm], params[:longest_flow], params[:wordcount])
            redirect '/'
        end
        ...
        get "#{path}/edit", auth: :writer do
            ...
            params[:wordcount] = 500 if params[:wordcount].to_s == ''
            params[:timer] = 5 if params[:timer].to_s == ''
